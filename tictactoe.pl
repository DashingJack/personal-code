...
full(A) :- x(A).
full(A) :- o(A).
/*this includes conditions for filling square with either x or o*/
empty(A) :- \+(full(A)).
/*this is condition for not filling slot of x with o*/
same(A, A). different(A, B) :- \+(same(A, B)).
/*This is condition for not filling by mistake x with x or o with o*/

move(C) :- rule1(C).
move(A) :- good(A), empty(A), !.
/*this are used to check if given move is valid and if so then do that move*/
good(1). good(3). good(7). good(9). good(5). good(2). good(4). good(6). good(8).
...
/*this is used to define what should be ticked by machine in it's move*/
...
/*this are basic rules to control the gameflow and showing the game to user*/
...
/*this includes rules to call all the rules or clauses to run the game*/
